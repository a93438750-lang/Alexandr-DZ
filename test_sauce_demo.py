...
class TestSauceDemoFunctionality:
    ...
    @allure.description(
        "Тест проверяет полный цикл покупки: авторизация, добавление товаров "
        "в корзину, оформление заказа и проверка итоговой суммы."
    )
    ...
    def test_shop_functionality(self, driver):
        ...
        with allure.step("Проверка, что итоговая сумма совпадает с ожидаемой"):
            assert total_price == EXPECTED_TOTAL, \
                f"Ожидаемая сумма {EXPECTED_TOTAL}, но получена {total_price}"
